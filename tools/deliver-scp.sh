#!/usr/bin/env bash
# usage: deliver-scp <src> <user@host> <remote-path> [--chmod=0755] [--owner=user] [--group=group] [--sudo]
set -euo pipefail
...
ssh -o IdentitiesOnly=yes "$host" "set -e; umask 022;
  dir='$dir'; tmp='$tmp'; mode='$mode'; chown_arg='$chown_arg'; SUDO='$SUDO';
  mkdir -p \"\$dir\";
  if [ -n \"\$mode\" ]; then \$SUDO chmod \"\$mode\" \"\$tmp\"; fi
  if [ -n \"\$chown_arg\" ]; then \$SUDO chown \"\$chown_arg\" \"\$tmp\"; fi
  \$SUDO mv -f \"\$tmp\" '$rpath'
"
echo "[deliver-scp] done -> $host:$rpath"
