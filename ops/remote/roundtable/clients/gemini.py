...
@guard_external("'gemini.py'")
async def 
    ensure_external_allowed("gemini.py")vote(task_text: str) -> Optional[Dict[str, Any]]:
    key = os.getenv("GOOGLE_API_KEY")
    if not key:
        return _mock(task_text)

    prompt = (
        f'You are Gemini. Evaluate the task "{task_text}" (0-10). '
        'ONLY JSON: {"ai_name":"Gemini","scores":{"speed":int,"quality":int,"creativity":int,"confidence":int},"why":"<=100","needed_inputs":[],"constraints":[]}'  # noqa: E501
    )
