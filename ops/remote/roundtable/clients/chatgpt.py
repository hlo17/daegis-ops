...
@guard_external("'chatgpt.py'")
async def 
    ensure_external_allowed("chatgpt.py")vote(task_text: str) -> Optional[Dict[str, Any]]:
    key = os.getenv("OPENAI_API_KEY")
    if not key:
        return _mock(task_text)

    prompt = (
        f'You are ChatGPT. Evaluate the task "{task_text}" (0-10 for speed/quality/creativity/confidence). '
        'ONLY JSON: {"ai_name":"ChatGPT","scores":{"speed":int,"quality":int,"creativity":int,"confidence":int},"why":"<=100","needed_inputs":[],"constraints":[]}'  # noqa: E501
    )
