...
class ProposalSummary(BaseModel):
    ...
    @validator('risks')
    def _v_risks(cls,v):
        if any(len(s)>50 for s in v): raise ValueError('risk too long'); return v
    @validator('dependencies')
    def _v_deps(cls,v):
        if any(len(s)>50 for s in v): raise ValueError('dep too long'); return v
    @validator('proposal_summary')
    def _v_len(cls,v,values):
        bl=len(v.encode('utf-8'))
        if not (300<=bl<=400): raise ValueError(f'proposal_summary must be 300-400 bytes, got {bl}')
        if 'total_length' in values: values['total_length']=bl
        return v
...
def compress_proposal(task_text: str, ai_name: str, raw_text: str) -> ProposalSummary:
    ...
    if bl>400: summary=summary.encode('utf-8')[:400].decode('utf-8','ignore')
    if bl<300: summary=summary+(" 充足性確認。"* ((300-bl)//15))
    total=len(summary.encode('utf-8'))
    return ProposalSummary(
        ai_name=ai_name, proposal_summary=summary, purpose=purpose,
        steps=steps, risks=risks, dependencies=dependencies,
        verification=verification, total_length=total)

def arbitrate(compressed_list: List[ProposalSummary], speed_priority: bool, quality_priority: bool, task_description: str) -> SynthesizedOutput:
    if not compressed_list: raise ValueError("no proposals")
    diff = [DifferencesTableItem(
    ...
    rejected=[RejectedItem(ai_name=p.ai_name,item=(p.steps[0] if p.steps else "alt"),reason="lower score") for p in compressed_list[1:]]
    ...
    ew=EvaluationWeights(speed_weight=sw,quality_weight=qw,other_weight=0.2,
                         formula_used="(speed?0.7:0.3)*speed+(quality?0.7:0.3)*quality+0.2*(creativity+confidence)/2")
    return SynthesizedOutput(differences_table=diff, adopted_items=adopted, rejected_items=rejected,
                             synthesized_proposal=s[:500], evaluation_weights=ew)
