...
def arbitrate(
    compressed_list: list[ProposalSummary],
    speed_priority: bool,
    ...
    adopted = [
        AdoptedItem(
            ai_name=compressed_list[0].ai_name, item=compressed_list[0].steps[0], reason="alignment"
        )
    ]
    rejected = [
        RejectedItem(
            ai_name=p.ai_name, item=(p.steps[0] if p.steps else "alt"), reason="lower score"
        )
