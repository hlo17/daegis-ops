...
def validate_schema(data: dict[str, Any]) -> bool:
    ...
    if not all(isinstance(sc.get(k, -1), int) and 0 <= sc[k] <= 10 for k in keys):
        return False
    return (
        len(data["why"]) <= 100
        and isinstance(data.get("needed_inputs", []), list)
        and isinstance(data.get("constraints", []), list)
    )


async def mock_vote(agent: str, task_text: str) -> dict[str, Any] | None:
    await asyncio.sleep(random.uniform(0.05, 0.2))
    vote = {
        "ai_name": agent,
        "scores": {
            k: random.randint(0, 10) for k in ["speed", "quality", "creativity", "confidence"]
        },
        "why": "mock",
    }
...
def select_coordinator(
    votes: list[dict[str, Any]], hist: dict[str, dict[str, float]], alpha: float = 0.6
) -> str | None:
    if not votes:
        return None
    ...
    for v in votes:
        ...
        h = _hist_score(
            hist.get(ai, {"lead_time": 300, "rework_count": 3, "user_rating": 1.0}), maxv
        )
    ...
    c2 = [
        a
        for a in cands
        if abs(hist.get(a, {"user_rating": 1.0}).get("user_rating", 1.0) - max_ur) < 1e-6
    ]
    import random as _r
